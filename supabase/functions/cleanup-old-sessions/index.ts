...
async function cleanupOldSessions() {
  try {
    ...
    console.log(`Found ${inactiveSessions?.length || 0} inactive sessions to cleanup`);
...
async function sendFollowUpMessages() {
  try {
    ...
    for (const user of inactiveUsers || []) {
      const messageIndex = Math.min(user.follow_up_count || 0, followUpMessages.length - 1);
      ...
      if (result && result.ok) {
        ...
        console.log(`Sent follow-up ${user.follow_up_count + 1} to user ${user.telegram_id}`);
      }
    }
