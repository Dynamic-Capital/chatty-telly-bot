...
async function getAutoReply(contentKey: string, variables: Record<string, string> = {}): Promise<string | null> {
  try {
    ...
    const now = Date.now();
    const cached = autoReplyCache.get(contentKey);
    if (cached && cached.expires > now) {
      return formatContent(cached.value, variables);
    }

    const template = await getAutoReplyTemplate(contentKey);
    if (!template) {
      console.log(`❌ No auto reply found for: ${contentKey}`);
      return null;
    }

    autoReplyCache.set(contentKey, { value: template, expires: now + CACHE_TTL });
    return formatContent(template, variables);
  } catch (error) {
...
const autoReplyCache = new Map<string, { value: string; expires: number }>();
