...
// ✅ Good Message Format
function formatSuccessMessage(title: string, details: string[], actions?: string[]): string {
  return `✅ ${title}
...
async function handleAIQuery(chatId: number, userId: string, query: string): Promise<void> {
  try {
...
async function efficientDataRetrieval(userIds: string[]): Promise<Map<string, any>> {
...
async function setupTestUser(userId: string, options: Partial<BotUser> = {}): Promise<BotUser> {
...
async function cleanupTestData(userId: string): Promise<void> {
  ...
  await supabaseAdmin.from('user_sessions').delete().eq('telegram_user_id', userId);
  await supabaseAdmin.from('user_interactions').delete().eq('telegram_user_id', userId);
}
```
